...
define mazut::rsync::simple (
  ...
  file { "xinetd-rsync-$title":
    ...
    group   => 'root',
    mode    => '0644',
    content => template('mazut/xinetd-service.erb'),
  }
  ->
  file { "rsync-conf-$title":
    ...
    group   => 'root',
    mode    => '0644',
    content => template('mazut/rsync-conf.erb'),
  }
  ->
